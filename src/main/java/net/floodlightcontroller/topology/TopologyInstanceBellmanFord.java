...
public class TopologyInstanceBellmanFord {
	...
	protected static Logger log = LoggerFactory
			.getLogger(TopologyInstanceBellmanFord.class);
	...
	protected class PathCacheLoader extends CacheLoader<RouteId, Route> {
		TopologyInstanceBellmanFord ti;

		PathCacheLoader(TopologyInstanceBellmanFord ti) {
			this.ti = ti;
		}
	...
	public TopologyInstanceBellmanFord() {
	...
	public TopologyInstanceBellmanFord(Map<Long, Set<Short>> switchPorts,
	...
	protected class NodeDist implements Comparable<NodeDist> {
		...
		private TopologyInstanceBellmanFord getOuterType() {
			return TopologyInstanceBellmanFord.this;
		}
	}

	protected BroadcastTree bellmanford(Cluster c, Long root,
			Map<Link, Integer> linkCost, boolean isDstRooted) {

		log.info("Running Bellman Ford algorithm for root:"+root);

		HashMap<Long, Link> nexthoplinks = new HashMap<Long, Link>();
		HashMap<Long, Integer> cost = new HashMap<Long, Integer>();

		// Initialize distances from src to all other vertices as MAX_PATH_WEIGHT		
		...
		for (int i = 0; i <= c.getNodes().size(); i++) {
			for (Map.Entry<Long, Set<Link>> links : c.links.entrySet()) {
				for (Link e : links.getValue()) {
					long u = links.getKey();
					long v = e.getDst();
					if (isDstRooted == true)
						v = e.getSrc();
					...
					int weight = 1;
					if (linkCost != null && linkCost.get(e) != null)
						weight = linkCost.get(e);

					if ((cost.get(u) != MAX_PATH_WEIGHT)
							&& ((cost.get(u) + weight) < cost.get(v))) {
						cost.put(v, cost.get(u) + weight);
						nexthoplinks.put(v, e);
					}
				}
			}
		}
		...
		for (Map.Entry<Long, Set<Link>> links : c.links.entrySet()) {
			for (Link e : links.getValue()) {
				...
				if ((cost.get(u) != MAX_PATH_WEIGHT)
						&& ((cost.get(u) + weight) < cost.get(v)))
					log.error("Graph contains negative weight cycle");

			}
		}

		log.info("Shortest paths:");
		for (long v : c.getNodes()) {
			if (nexthoplinks.containsKey(v))
				log.info("node->next:" + nexthoplinks.get(v).getSrc() + "->"
						+ nexthoplinks.get(v).getDst());
			log.info("Cost of " + v + ":" + cost.get(v));
		}
	...
	protected void calculateShortestPathTreeInClusters() {
		...
		for (Cluster c : clusters) {
			for (Long node : c.links.keySet()) {
				BroadcastTree tree = bellmanford(c, node, linkCost, true);
